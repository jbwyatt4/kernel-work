...
for msg in msg_it:
	...
	if event.name == checked_events[2]:
		if sched < 5:
			msg = 'Event: {} | Prev Cookie {} | Next Cookie {} | Core Cookie {}'
			print(msg.format(event.name, event.payload_field["prev_cookie"], event.payload_field["next_cookie"], event.payload_field["core_group_cookie"]))
		sched = sched + 1
